...
class ModelHelper():
    ...
    def create_model(self, x_images, keep_prob):
        # 转灰度
        # imgFile = tf.image.decode_jpeg(imgFile, channels=3)
        # x_images = tf.image.rgb_to_grayscale(x_images)

        # first layer
        w_conv1 = self.weight_variable([5, 5, 1, 32])
        b_conv1 = self.bias_variable([32])
        h_conv1 = tf.nn.relu(tf.nn.bias_add(
            self.conv2d(x_images, w_conv1), b_conv1))
        ...
        h_conv2 = tf.nn.relu(tf.nn.bias_add(
            self.conv2d(h_dropout1, w_conv2), b_conv2))
        ...
        h_conv3 = tf.nn.relu(tf.nn.bias_add(
            self.conv2d(h_dropout2, w_conv3), b_conv3))
