...
class SimilarityModel():
    ...
    def create_model_one(self, input_value):
        ...
        # 压缩维度
        inputs = K.layers.Conv2D(256, (3, 3),
        ...
        inputs = K.layers.Conv2D(256, (int(inputs.shape[1]), int(inputs.shape[2])),
                                 activation=K.activations.relu,
                                 name="SimilarityModel_Conv2", padding="valid")(inputs)
        inputs = K.layers.Conv2D(128, (1, 1),
        ...
        inputs = K.layers.Lambda(lambda x: tf.reshape(x, (-1, 128)))(inputs)
